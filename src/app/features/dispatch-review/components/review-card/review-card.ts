...
export class ReviewCard {
  ...
  /**
   * One click from a scroll, so it asks once. Archiving is reversible, but a
   * card silently vanishing under the cursor is not the way to find that out.
   */
  readonly confirmingArchive = signal(false);
  toggleConfirmArchive(): void { this.confirmingArchive.update(v => !v); }
  confirmArchive(): void { this.confirmingArchive.set(false); this.archive.emit(); }
